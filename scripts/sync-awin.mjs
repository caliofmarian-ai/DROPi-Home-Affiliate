...
async function fetchJson(url, options) {
  ...
  if (!response.ok) throw new Error(`Awin API ${response.status}: ${body.slice(0, 300)}`);
...
export async function syncAwin({
  ...
  const feedResponse = await fetchImpl(feedUrl, { headers: { Authorization: `Bearer ${accessToken}` } });
  ...
  for (const offer of offers) {
    ...
    if (!response.ok) throw new Error(`Awin Link Builder failed with ${response.status}: ${text.slice(0, 250)}`);
    let payload;
    try { payload = JSON.parse(text); } catch { throw new Error('Awin Link Builder returned invalid JSON.'); }
  ...
  return { skipped: false, feedProducts: rows.length, mappedOffers: offers.length, offers };
}

if (process.argv[1] && resolve(process.argv[1]) === resolve(new URL(import.meta.url).pathname)) {
  syncAwin().then(result => {
    if (result.skipped) console.log('SKIP: Awin is not configured; no files changed.');
    else console.log(`PASS: Awin feed synced; ${result.feedProducts} feed products, ${result.mappedOffers} exact mapped offers.`);

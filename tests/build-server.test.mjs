...
test('fully synthetic approved public build excludes owner workspace and includes sitemap', () => { const root = fixture(); try { approveSyntheticFixture(root); const b = build({ root, env: { RELEASE_MODE: 'public', SITE_ORIGIN: 'https://fixture.example.org' }, now: fixed }); assert.equal(b.pages, 18); assert.equal(b.routes['/ops/'], undefined); assert.ok(b.routes['/sitemap.xml']); const text = readFileSync(resolve(root, 'dist/index.html'), 'utf8'); assert.doesNotMatch(text, /Owner workspace|PRIVATE-PREVIEW/); assert.match(text, /index,follow/); assert.match(text, /1 Fixture Street/); } finally { rmSync(root, { recursive: true }); } });

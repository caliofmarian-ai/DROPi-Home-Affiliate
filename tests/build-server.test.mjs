...
test('preview builds 19 pages with no private configuration or tracker', () => {
  const root = fixture(); try { const b = build({ root, env: {}, now: fixed }); assert.equal(b.pages, 19); assert.equal(b.guides, 10); assert.equal(b.products, 16); assert.ok(b.routes['/ops/']); assert.equal(existsSync(resolve(root, 'dist/data/site.json')), false); const home = readFileSync(resolve(root, 'dist/index.html'), 'utf8'); assert.match(home, /noindex,nofollow/); assert.doesNotMatch(home, /posthog|gtag\(|tag=|awinaffid=/); assert.match(readFileSync(resolve(root, 'dist/robots.txt'), 'utf8'), /Disallow: \//); } finally { rmSync(root, { recursive: true }); }
});
...
test('fully synthetic approved public build excludes owner workspace and includes sitemap', () => { const root = fixture(); try { approveSyntheticFixture(root); const b = build({ root, env: { RELEASE_MODE: 'public', SITE_ORIGIN: 'https://fixture.example.org' }, now: fixed }); assert.equal(b.pages, 18); assert.equal(b.routes['/ops/'], undefined); assert.ok(b.routes['/sitemap.xml']); const text = readFileSync(resolve(root, 'dist/index.html'), 'utf8'); assert.doesNotMatch(text, /Owner workspace|PRIVATE-PREVIEW/); assert.match(text, /index,follow/); } finally { rmSync(root, { recursive: true }); } });
...
test('HTTP page, assets, 404 and security headers', async () => {
  const root = fixture(); let app; try { build({ root, env: {}, now: fixed }); app = createApp({ root: resolve(root, 'dist') }); const base = await listen(app);
    const home = await fetch(base); assert.equal(home.status, 200); assert.match(await home.text(), /A little more room/); assert.equal(home.headers.get('x-content-type-options'), 'nosniff'); assert.equal(home.headers.get('set-cookie'), null); assert.match(home.headers.get('content-security-policy'), /frame-ancestors 'none'/);
...
test('network-exposed preview refuses to start without strong auth', () => { const root = fixture(); try { build({ root, env: {}, now: fixed }); assert.throws(() => createApp({ root: resolve(root, 'dist'), host: '0.0.0.0' }), /PREVIEW_TOKEN/); assert.throws(() => createApp({ root: resolve(root, 'dist'), token: 'short' }), /32/); } finally { rmSync(root, { recursive: true }); } });
